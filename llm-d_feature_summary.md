...
| # | Feature (llm-d) | Category | Description | Comparable feature in NVIDIA Dynamo | Open PRs |
|---|---------|----------|-------------|-------------------------------------|----------|
| 1 | [**Optimized Baseline**](./guides/optimized-baseline/README.md) | Intelligent Routing | Deploy vLLM with prefix-cache and load-aware routing enabled by the llm-d EPP (Endpoint Picker). | **KV Cache Aware Routing** (Router) — routes on worker load + KV-cache overlap to cut redundant prefill (~2× faster TTFT). | [#1683](https://github.com/llm-d/llm-d/pull/1683) — Add lm-eval accuracy evaluation workflow to the optimized-baseline guide. |
...
| 5 | [**Tiered Prefix Cache**](./guides/tiered-prefix-cache/README.md) | KV-Cache Management | Offloads KV caches beyond accelerator memory (to CPU/disk), increasing KV working-set size for multi-turn request patterns. | **KV Block Manager (KVBM)** — offloads KV cache GPU → CPU → SSD → remote/object storage (S3/Azure); LMCache/FlexKV integrations. | [#1230](https://github.com/llm-d/llm-d/pull/1230) — Support tiered prefix cache on Intel XPU via an offloading connector. |
...
| 9 | [**Workload Autoscaling**](./guides/workload-autoscaling/README.md) | Operational Excellence | Proactive, SLO-aware autoscaling using queue depth, in-flight counts, and KV-cache pressure to add capacity before latency degrades. | **Planner** — SLA-driven autoscaler that profiles workloads and right-sizes prefill/decode pools to meet TTFT/ITL targets. | [#1224](https://github.com/llm-d/llm-d/pull/1224) — Add an Intel XPU walkthrough for the HPA + EPP autoscaling well-lit path. |
...
| 12 | [**Multimodal Serving**](./guides/multimodal-serving/README.md) | Workloads | Deploys multimodal (image/audio/video) serving via aggregated routing or dedicated encode-disaggregation topologies. | **Multimodal Serving** — native multimodal with embedding cache; also video generation (FastVideo / SGLang Diffusion). | [#1957](https://github.com/llm-d/llm-d/pull/1957) — Enable the XPU vLLM-Omni multimodal aggregation guide and image build pipeline. |
...
| 15 | [**Encode Disaggregation**](./guides/multimodal-serving/e-disaggregation/README.md) | Experimental | Offloads multimodal encoding to dedicated workers via E/PD or E/P/D topologies, freeing prefill/decode resources for text. | **Disaggregated encode/prefill/decode** for multimodal (E/PD, E/P/D) with embedding cache. | [#1853](https://github.com/llm-d/llm-d/pull/1853) — Add a Wan text-to-video XPU Encoder/Generator (EG) disaggregated model server. |
